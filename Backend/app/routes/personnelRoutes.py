...
from Backend.app.controllers.personnelController import add_personnel_to_event, get_personnel_for_event, update_personnel_to_event, delete_personnel_to_event
...
@router.put("/update/{student_code}")
async def update_personnel_route(student_code: str, personnel_data: dict, db: Session = Depends(get_db)):
    return update_personnel_to_event(student_code, personnel_data, db)
# #Xóa nhân sự.
@router.delete("/delete/{student_code}")
async def delete_personnel_route(student_code: str, db: Session = Depends(get_db)):
    return delete_personnel_to_event(student_code, db)
